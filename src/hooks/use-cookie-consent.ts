...
export function useCookieConsent() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("tatli:cookie-consent", handler as EventListener);
    return () => {
      window.removeEventListener("storage", handler);
      window.removeEventListener("tatli:cookie-consent", handler as EventListener);
    };
  }, []);
  ...
  const api = useMemo(() => {
    return {
      ...
      acceptNecessaryOnly: () => {
        ...
        window.dispatchEvent(new Event("tatli:cookie-consent"));
      },
      acceptAll: () => {
